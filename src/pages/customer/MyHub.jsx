...
export default function MyHub() {
  ...
  return (
    <div className="flex flex-col gap-0 pb-8">
      {/* Hero card */}
      <div className="mx-5 mt-5 rounded-md bg-ink text-white overflow-hidden">
        <div className="px-5 pt-5 pb-4">
          <div className="flex items-start justify-between mb-4">
            <div>
              ...
              <p className="text-3xl font-bold tracking-tight font-mono">KES {availableCredit.toLocaleString()}</p>
            </div>
            <div className="text-right">
              <p className="text-[11px] text-white/50 uppercase tracking-wider font-medium mb-1">SokoPoints</p>
              <p className="text-2xl font-bold font-mono text-accent">{sokoPoints}</p>
            </div>
          </div>
          ...
          {hasActiveLoan && (
            <button
              ...
              className="w-full py-2 rounded bg-white/10 border border-white/20 text-xs font-semibold text-white/80 hover:bg-white/15 transition-colors flex items-center justify-center gap-2"
            >
      ...
      {isFirstTimer && (
        <div className="mx-5 mt-4 p-4 rounded-md bg-surface border border-border">
      ...
      <div className="mx-5 mt-4 p-4 rounded-md bg-surface border border-border">
        ...
        <div className="flex gap-4 mb-4">
          <div className="flex-1 text-center p-3 rounded bg-ground">
            <p className="text-xl font-bold font-mono text-status-paid-text">{onTimeRate}%</p>
            <p className="text-[10px] text-ink-muted mt-0.5">On-time rate</p>
          </div>
          <div className="flex-1 text-center p-3 rounded bg-ground">
            <p className="text-xl font-bold font-mono text-ink">{completedCycles}</p>
            <p className="text-[10px] text-ink-muted mt-0.5">Completed cycles</p>
          </div>
          <div className="flex-1 text-center p-3 rounded bg-ground">
            <p className="text-xl font-bold font-mono text-ink">{reliability}</p>
            <p className="text-[10px] text-ink-muted mt-0.5">Reliability</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2.5">
          {[
            ...
            <button
              ...
              className="flex flex-col items-center gap-1.5 py-3.5 rounded border border-border bg-surface text-[11px] font-medium text-ink-dim hover:bg-ground transition-colors"
            >
